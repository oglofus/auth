...
export const successResult = <U extends UserBase>(user: U, sessionId: string, issues: Issue[] = []): AuthResult<U> => ({
  ok: true,
  user,
  sessionId,
  issues,
...
export const successOperation = <TData>(data: TData, issues: Issue[] = []): OperationResult<TData> => ({
  ok: true,
  data,
  issues,
});
